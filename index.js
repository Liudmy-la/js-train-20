...
function Truck(
  ...
  //  Записуєм в this.color значення аргументу color, в this.engineType значення аргументу engineType і так далі зі всіми аргументами
		...
		// для спадкування властивостей та методів з Vehicle в Truck.
...
Truck.prototype.specific = function(num) {
	if (num > this.towingCapacity) {
		...
		console.log(`Тягнення навантаження ${this.towingCapacity}`);
	}
};
...
const bindDrive = car.drive.bind(car);
...
function ElectricCar(brand, model, year, mileage, batteryCapacity) {
  ...
  if (!!new.target) {
	...
	} else console.error(`Конструктор має бути викликаний з 'new'`);

}

		// для спадкування властивостей та методів з Car в ElectricCar.
